...
/*Fonction qui vérifie qu'il n'y a que des espaces, des 0, des 1, et soit S, N, W, E dans la map*/
static int	check_chars_in_map(t_game *game)
{
	...
	while (game->tab[++i])
	{
		...
		while (game->tab[i][j])
		{
			if (game->tab[i][j] != ' ' && game->tab[i][j] != '0' && game->tab[i][j] != '1' && game->tab[i][j] != 'S' && game->tab[i][j] != 'N' && game->tab[i][j] != 'W' && game->tab[i][j] != 'E')
...
static int	check_player(t_game *game)
{
	...
	while (game->tab[i])
	{
		...
		while (game->tab[i][j])
		{
			if (game->tab[i][j] == 'N' || game->tab[i][j] == 'S' || game->tab[i][j] == 'E' || game->tab[i][j] == 'W')
			{
				if (game->player.x != 0)
